...
class WeightedDataLoader:
    ...
    def get_dataset_chunk(self):
        MAX_CHUNK_SIZE = (2**24-1) // len(self.dataset_ls)
...
def get_train_eval_datasets(data_config:DataTrainingArguments,
    ...
    def get_tgt(dom, data_dict):
        if "tgt" in data_dict.keys() and dom in data_dict["tgt"]:
            return data_dict["tgt"][dom]
        return data_dict["train"][dom]
    
    train_dict = {dom:data_dict['train'][dom] for dom in train_domains if dom in data_dict['train']}
    tgt_dict = {dom:get_tgt(dom, data_dict=data_dict) for dom in tgt_domains if (dom in data_dict["train"] or dom in data_dict["tgt"])}
