...
def get_wiki40b(subset='en', num_proc=40,
    ...
    tgt_path = os.path.join(SUBSET_PATH, f"{subset}_validation.bin")
    
    train_data = np.memmap(train_path, dtype=np.int32, mode='r')
    test_data = np.memmap(test_path, dtype=np.int32, mode='r')
    tgt_data = np.memmap(tgt_path, dtype=np.int32, mode='r')
    print(f'Subset {subset}: train[{len(train_data)}] | val[{len(test_data)}] | tgt[{len(tgt_data)}]')
    if return_torch:
        train_data = torch.tensor(np.array(train_data, dtype=np.int32))
        test_data = torch.tensor(np.array(test_data, dtype=np.int32))
        tgt_data = torch.tensor(np.array(tgt_data, dtype=np.int32))
    return {'train': train_data, 'val': test_data, 'tgt': tgt_data}
