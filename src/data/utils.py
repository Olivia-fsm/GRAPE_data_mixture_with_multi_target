...
from .climb import get_climblab
...
def get_benchmark_data(dataset_name:str = "arc_easy",
                       num_proc: int = 10,
                       pad_to_multiple=1024) -> Dict[str, np.ndarray]:
    """
    ...
    rst_dict = data_func(num_proc=num_proc, pad_to_multiple=pad_to_multiple)
    train_ood = 20000*pad_to_multiple
    rst_dict["train"] = rst_dict["train"][:min(train_ood, len(rst_dict["train"]))]
    return rst_dict
...
def get_dataset(args, dataset=None) -> Dict[str, np.ndarray]:
    ...
    if 'wiki40b' in trg_dataset:
        ...
        for subset in subset_list:
            ...
            rst_dict['tgt'][subset] = subset_data['tgt'][-n_tgt:]
            print(f"Subset {subset}: train[{len(subset_data['train'])}]|val[{len(subset_data['val'])}]|tgt[{len(subset_data['tgt'])}]")
        return rst_dict
    elif 'climb' in trg_dataset:
        subset_ood = trg_dataset.split('-')[1:]
        rst_dict = {}
        rst_dict['train'] = {}
        rst_dict['val'] = {}
        n_items_train = 5000000000
        # n_items_train = 500000000
        n_items_train_ood = 50000000
        n_items_val = 5000
        
        for k in range(1,21):
            subset_data = get_climblab(subset=f"cluster_{k}", num_proc=10)
            if k in subset_ood:
                rst_dict['train'][f"cluster_{k}"] = subset_data['train'][:min(n_items_train,len(subset_data['train']))]
                rst_dict['val'][f"cluster_{k}"] = subset_data['val'][:n_items_val*args.max_token_length]
            else:
                rst_dict['train'][f"cluster_{k}"] = subset_data['train'][:min(n_items_train,len(subset_data['train']))]
                rst_dict['val'][f"cluster_{k}"] = subset_data['val'][:n_items_val*args.max_token_length]
        ...
        for s in subset_ood:
            if s in rst_dict['train'].keys():
                continue
            try:
                subset_data = get_benchmark_data(dataset_name=s, pad_to_multiple=args.max_token_length)
            except:
