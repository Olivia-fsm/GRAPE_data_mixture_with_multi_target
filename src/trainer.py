...
class FullTrainingArguments(TrainingArguments):
    ...
    reweight_tgt: str = field(
        default="grape", metadata={"help": "Reweighting target domains. [grape; grape_gap; grape_ema; xx_grad: manipulate graduents; None: fixed.]"}
    )
...
class MAPTrainer(Trainer):
    ...
    def get_domain_grad(self, individual_inputs, 
        ...
        if not return_grad:
            return loss_value.detach().cpu().item()
        # compute gradient
    ...
    def domain_reweight_step(self, 
        ...
        # get_tgt_log_grad = (self.reweight_tgt == "grape")
        get_tgt_log_grad = (self.reweight_tgt == "grape") or (self.reweight_tgt == "grape_ema")
        ...
        for idx, domain_id in enumerate(self.train_ids):
            ...
            domain_loss, domain_grad = self.get_domain_grad(individual_train_inputs, domain_id=idx, get_log_grad=False, 
                                                            return_loss=True, return_grad=True)
            ...
            # if lr_value < 1e-4: 
            #     lr_value = 1e-4
            log_train_dw[idx] = torch.log(self.train_dw[idx]) + self.mu_train * (domain_grad @ tgt_grad) 
        del domain_grad
        del tgt_grad
    ...
    def tgt_reweight_step_offline(self,
        ...
        for idx, domain_id in enumerate(self.tgt_ids):
            domain_name = self.idx2domain[domain_id]
            domain_loss = self.get_domain_grad(individual_tgt_inputs,
                                                domain_id=idx,
                                                get_log_grad=False,
                                                return_loss=True,
                                                return_grad=False)
            ...
            last_step_loss = self.tgt_tracker_dict[domain_name].get_last_loss(step=None)
            if last_step_loss is None:
                roi_score = 0.
            else:
                roi_score = (domain_loss - last_step_loss) / last_step_loss
            ...
            wandb_log_dict[f'ema_reweight_loss/{domain_name}'] = ema_domain_loss
            log_tgt_dw[idx] = torch.log(self.tgt_dw[idx]) - self.mu_tgt * (roi_score) # eliminate lr_value
            wandb_log_dict[f"offline_roi_score/{domain_name}"] = roi_score
        tgt_dw = torch.nn.functional.softmax(log_tgt_dw, dim=-1)
        tgt_dw = tgt_dw * (1 - self.reweight_eps) + self.reweight_eps / len(tgt_dw) # smoothing
    ...
    def tgt_reweight_step(self, 
                          ...
                          lr_value):
        # TODO: implement doge with re-sampling
        if self.reweight_tgt == "grape_offline":
            self.tgt_reweight_step_offline(individual_tgt_loader,
                                        mix_train_iter,
                                        lr_value)
            return
        wandb_log_dict = {}
        ...
        for train_input in train_inputs:
            if train_grad is not None:
                ...
                train_grad = self.get_domain_grad(train_input, domain_id=None, get_log_grad=False,
                                                  return_loss=False, return_grad=True)
        train_grad = train_grad / num_batches
        individual_tgt_inputs = individual_tgt_loader.__next__()
        log_tgt_dw = torch.zeros_like(self.tgt_dw)
        ...
        for idx, domain_id in enumerate(self.tgt_ids):
            ...
            wandb_log_dict[f'reweight_loss/{domain_name}'] = domain_loss
            wandb_log_dict[f'ema_reweight_loss/{domain_name}'] = ema_domain_loss
            if self.reweight_tgt == "grape_ema":
                domain_grad = domain_grad / ema_domain_loss
            log_tgt_dw[idx] = torch.log(self.tgt_dw[idx]) - self.mu_tgt * (domain_grad @ train_grad) # eliminate lr_value
        del domain_grad
        del train_grad
        tgt_dw = torch.nn.functional.softmax(log_tgt_dw, dim=-1)
        tgt_dw = tgt_dw * (1 - 0.001) + 0.001 / len(tgt_dw) # smoothing
        self.tgt_dw_update_steps += 1
        self.tgt_dw = tgt_dw
        self.avg_tgt_dw += tgt_dw
        for idx, domain_id in enumerate(self.tgt_ids):
